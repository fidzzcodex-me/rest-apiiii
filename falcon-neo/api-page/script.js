...
// Update hero stats
function updateHeroStats(apis) {
  let total = 0, cats = 0, ready = 0;
  if (apis && apis.categories) {
    cats = apis.categories.length;
    apis.categories.forEach(cat => {
      if (cat.apis) {
        total += cat.apis.length;
        cat.apis.forEach(api => { if (!api.status || api.status === 'ready') ready++; });
      }
    });
  }
  const te = document.getElementById('stat-total-api');
  const ce = document.getElementById('stat-categories');
  const re = document.getElementById('stat-ready');
  if (te) te.textContent = total || '-';
  if (ce) ce.textContent = cats || '-';
  if (re) re.textContent = ready || '-';
}
